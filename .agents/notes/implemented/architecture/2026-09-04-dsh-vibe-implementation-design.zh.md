...
可分发形态确定为 Git tag 包 `@maocili/vibe-init`。包 manifest 声明指向包内 `cordis.patch.yml` 的
`dsh.bundle.patch`；DSH profile 安装 bundle 后，该 patch 将同包作为 Cordis plugin 插入。`files` 白名单只携带
运行时、CLI、patch 和 `packages/`；`pnpm pack` 负责验证 tarball，安装生命周期脚本不修改项目或 profile。

原设计文档现在只保留指针；本笔记负责承载实现决策及其理由。稳定范围和行为见
[`REQUIREMENTS`](../../../../docs/REQUIREMENTS-vibe-init.md)，证据见
