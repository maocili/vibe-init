...
The implementation is organized as a deterministic `plan` → `evaluate` → `apply` pipeline. `packages/` is the only versioned source for rule, note-skeleton, skill, feature, and toolchain content; `manifest.json` records feature defaults, toolchain groups, file targets, and content hashes. Every manifest source, row target, and toolchain spec/target uses one safe-relative-path rule before pack I/O; pack-derived project targets are then verified to remain under the selected project root. Invalid pack rows are excluded and write commands fail before planning, while read-only commands do not materialize unsafe entries. The engine appends marker-wrapped rule blocks, materializes fresh notes and selected skills, and manages the project toolchain while preserving user-edited content as conflicts. Repeated runs converge without writes, and `status`/`audit` reuse the same read-only plan.
