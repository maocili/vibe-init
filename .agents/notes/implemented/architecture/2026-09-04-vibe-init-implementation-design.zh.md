...
`upgrade` 按所有权迁移。marker 段、生成文档、声明的工具链文件、声明的技能文件和笔记骨架说明文件由工具拥有，可刷新同步；日期命名的 Agent Note、笔记 manifest、业务文档和未知文件仍由项目拥有。只有状态确认拥有且当前内容仍匹配安装哈希的受管旧文件才会删除；修改或归属不明的路径保留并报告冲突。工具链依赖安装成功后才原子提交状态；安装失败时恢复升级前的组装 `package.json`，使重试的 upgrade 能再次检测到组合变化并重试安装，而不是在依赖缺失的情况下提交状态。`init` 只有在既有文件可被识别为本工具自己的组合（同名声明包名）时才刷新组装的工具链 `package.json`；受管 home 内的外来 `package.json` 是冲突，绝不覆盖。
