...
仍作为受管资产。仅显式调用的翻译 skill 也成为内置双语工具链文档与 briefing 注释使用的名称。通用 `pre-push-checks` skill 采用与 Harness 专用工作流相同的执行骨架：确定性检查工作树和基线、发现项目声明的 scope 与校验命令、划清 hook 边界、执行聚焦证据、保护重写推送、先正常提交再推送，并在推送后校验远端；项目专用命令仍按条件启用。根 marker、笔记骨架和随包工作流保留可移植的交叉链接；它们只指向项目内受管文件或随包安装的相邻 skill。
