...
Land dependent PRs through GitHub's native stack object and `gh stack merge`. Do not reproduce stack semantics by merging and retargeting individual PRs with `gh pr merge` and `gh pr edit`. The root [AGENTS.md](../../../.template/AGENTS.md) owns the allowed merge-forward and rebase histories; the stack review guide owns review-fix propagation.
