...
| [i18n/](i18n/README.md) | The bilingual pairing contract: equal-authority English/Chinese pairs and their consistency records | Translation content (→ each pair) |
| Skills (`.agents/skills/`) | Reusable workflows and specialized decision standards | Product and runtime contracts (→ docs or source) |

Placement: bugs → postmortems; rationale → Agent Notes; standing orders → root `AGENTS.md` with a rationale link.
...
- **Fenced `ts` blocks must compile** (`doc-typecheck`).
