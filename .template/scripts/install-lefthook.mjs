...
/** Template content directory relative to the Git root (this container's layout). */
...
const PAIRING_MERGE_DRIVER_CONFIG = [
  ...
  [
    ...
    `${TEMPLATE_PREFIX}/scripts/merge-translation-pairing-driver.sh %O %A %B %P`,
  ],
]
...
function probePairingMergeDriver(root) {
  capture(process.execPath, PAIRING_MERGE_DRIVER_PROBE, { cwd: join(root, TEMPLATE_PREFIX) })
}
...
async function main() {
  ...
  const lefthook = join(root, TEMPLATE_PREFIX, 'node_modules', '.bin', isWindows ? 'lefthook.cmd' : 'lefthook')
