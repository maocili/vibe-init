...
const pairingMergeDriver = '.template/scripts/merge-translation-pairing-driver.sh %O %A %B %P'
...
if (!shouldFail) {
  const binary = join(root, '.template', 'node_modules', '.bin', process.platform === 'win32' ? 'lefthook.cmd' : 'lefthook')
...
function installFakeLefthook(root: string): void {
  const binDirectory = join(root, '.template/node_modules/.bin')
...
function installPairingProbeFixture(root: string): void {
  ...
  mkdirSync(join(root, '.template/node_modules'), { recursive: true })
  symlinkSync(scriptsDirectory, join(root, '.template/scripts'), linkType)
  symlinkSync(tsxPackageDirectory, join(root, '.template/node_modules/tsx'), linkType)
}
...
describe('worktree-local Lefthook installer', { timeout: 15_000 }, () => {
  ...
  it('does not publish worktree integration when the pairing driver probe fails', async () => {
    const fixture = createFixture()
    rmSync(join(fixture.main, '.template/node_modules/tsx'), { recursive: true, force: true })
