...
tsx_loader=$driver_directory/../node_modules/tsx/dist/esm/index.mjs

if command -v node >/dev/null 2>&1 \
  && node --import "$tsx_loader" "$driver_path" --probe >/dev/null 2>&1; then
  exec node --import "$tsx_loader" "$driver_path" \
