# dsh-rules — 项目级初始化器 DSH 插件（仓库根 = 插件源码）

仓库根即插件本体（2026-09-04 由 `plugin/` 展平到根）：同仓含内置规则包 [`rules-pack/`](rules-pack/README.md)（唯一版本化内容源）与项目文档 [`docs/`](docs/README.md)（REQUIREMENTS/DESIGN/ACCEPTANCE/CHANGELOG/总览）。插件说明见下；仓库总览见 [docs/README.md](docs/README.md)。

> 状态：**M1 引擎 v1.0 + M1b docGates 工具链已实现**（纯 Node、零依赖，可独立运行与验收；仓库根 `pnpm test` 26/26）。
> 需求/范围口径：[`docs/REQUIREMENTS-dsh-rules-plugin.md`](docs/REQUIREMENTS-dsh-rules-plugin.md)（v1.0 定案）——**项目级初始化器，不触碰全局面**
> （~/.dsh/AGENTS.md、用户技能根永不写入）；实现方案细节以 [`docs/DESIGN-dsh-rules-plugin.md`](docs/DESIGN-dsh-rules-plugin.md) 为准
...
- **只做项目级安装/管理器**：物化 + 幂等 + 迁移；规则与技能文本一律来自版本化规则包 [`rules-pack/`](rules-pack/README.md)（本仓库）。
...
| 命令 | 作用对象 | 行为 |
|---|---|---|
| `init` | 当前项目 | 生成 `.agents/notes/` 骨架 + 根 `AGENTS.md` marker 规则段（note 纪律块 + 按 `manifest.features` 默认开启的特性段）+ docGates 工具链 `.dsh-rules/toolchain/`（按 feature 门控）+ `--skill` 挑选的技能副本 |
...
选项：`--project <dir>`（默认 cwd，向上找 `.git` 定界）· `--pack <dir>`（默认本仓库 `rules-pack/`，可 `$DSH_RULES_PACK` 覆盖）·
...
- **docGates 工具链**：manifest `toolchain` 声明 + `toolchain/spec.json` 分组；伞特性 `docGates`（默认 true）物化
  scaffold/常设门禁/挂钩，`textLinkManagement`/`docBudgets`/`bilingualDocsDiscipline`/`docGatesExtras` 门控各自脚本组；
  `package.json`（doc-sync 等）由引擎按启用组确定性组装（managedUpdate）；关闭组/伞 → 受管副本移除
  （字节等于源才删，用户改动报 conflict 保留，`--force` 可强制）。`postinstall`（install-lefthook.mjs）在消费者
  `pnpm install` 时把 pre-commit 挂钩写入 git 根（已有外来 lefthook.yml 时报告不覆盖）。
...
- **feature 默认值（v1.0 + M1b）**：`textLinkManagement`/`bilingualPairing`/`docBudgets`/`docGates` 默认开；`bilingualDocsDiscipline`、`docGatesExtras` 默认关。
  每次运行可用 `--feature <key>=<true|false>` 覆盖（按 key 合并、未覆盖者回落默认；关闭的段被移除），
  已冒烟验证默认态往返字节一致。
- **自动化测试**：仓库根 `pnpm test`（node:test；临时目录自清理，26 条）：引擎层——cleanSource 剥离、
...
1. **真实挂载（已执行，2026-09-03，用户同意）**：下列条目已并入 `~/.dsh/profiles/web/cordis.patch.yml`
   ...
   `[dsh-rules] mounted`（tmux `dsh:0.0`）；M2 在真实项目 `/Users/xuxifeng/Work/dsh-rules-demo` 验证通过。
   **（注意：2026-09-04 插件已展平到仓库根，旧条目 `…/plugin/dsh-rules.mjs` 路径失效；宿主需按下方新路径更新后重启 profile 重新挂载。）**
   条目（新路径）：
...
4. 后续开发：DP-F/DP-G 解除后实现（skills-optional 内容、自装识别/声明集）、M3 分发；`--feature` 按次覆盖暂不持久化（特性默认以 manifest 为准，`docGates=false` 一次运行即移除整个工具链）。
