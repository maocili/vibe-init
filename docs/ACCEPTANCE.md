...
> 范围权威：REQUIREMENTS-dsh-vibe-plugin.md（v1.0 定案）。未实现项均已标注（DP-G 仍待决定）。
...
| D11 docGates 伞（默认 true）+ docGatesExtras（默认 false）+ 每 feature 配脚本组（M1b 方案甲） | manifest features + `toolchain/spec.json`；toolchain.test.mjs（默认物化集/组门控/extras opt-in）；CLI 默认 init 物化 .dsh-vibe/toolchain | 达成（2026-09-04） |
...
| M2 真实挂载与验证 | **已通过（2026-09-03）**：insert 已写入（用户同意）；宿主重启日志出现 mounted；真实项目 /Users/xuxifeng/Work/dsh-vibe-demo init/status/audit 验证绿 |
| M3 可分发 | `package.json` 的 `@maocili/dsh-vibe` + `dsh.bundle.patch`、根 `cordis.patch.yml`；`test/package.test.mjs` 验证 pnpm tarball 清单、隔离全局安装、Cordis 入口和 CLI init/status | 达成（2026-09-05） |
