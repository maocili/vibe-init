...
| D8 | vibe-init 自有的 `AGENTS.md` 与投射到其中的受管规则段使用英文单语；根规则块只保留契约、红线、门禁和指向细则所有者的链接，不承载格式模板、双语摘要或微观工作流。marker 外的项目内容仍由用户拥有。 |
...
| R8 | `status`/`audit` 只读报告受管状态、漂移与受管面内意外内容；两者都不探测旧命名空间。 |
