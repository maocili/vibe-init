// dsh-vibe CLI — M1 command surface for the installer/manager plugin.
// Run directly (node bin/dsh-vibe.mjs …) or through the Cordis mount; actions are also
...
const USAGE = `dsh-vibe <command> [options]
...
function err(msg) { console.error('[dsh-vibe] ' + msg) }
...
async function reviewThenApply(label, plan, pack, opts) {
  ...
  if (opts.dryRun) {
    log('[dsh-vibe] ' + label + ' (dry-run): ' + summaryLine(summary, dirty.length))
  ...
  if (plan.mode === 'upgrade' && plan.stateInfo?.problem) {
    err(plan.stateInfo.problem + '; repair or remove .dsh-vibe/state.json before retrying')
  ...
  if (!pending.length) { log('[dsh-vibe] ' + label + ': nothing to do (already up to date)'); return { results, summary, applied: [], aborted: false, requiresInstall: !!plan.requiresInstall } }
  const go = opts.yes ? true : await confirm(label + ' — apply ' + pending.length + ' change(s), keep ' + blocked.length + ' conflict(s) untouched?')
  if (!go) { log('[dsh-vibe] ' + label + ': aborted by user — no files written'); return { results, summary, applied: [], aborted: true, requiresInstall: !!plan.requiresInstall } }
  const applied = await applyResults(results)
  log('[dsh-vibe] ' + label + ': applied ' + applied.length + ' change(s)')
...
export async function installToolchainIfNeeded(plan, projectRoot, runner = execFileAsync) {
  ...
  if (process.env.DSH_VIBE_SKIP_TOOLCHAIN_INSTALL === '1') {
    log('[dsh-vibe] toolchain dependency install skipped by test environment')
    return null
  }
  try {
    await runner('pnpm', ['-C', plan.toolchainHome, 'install'], { cwd: projectRoot, maxBuffer: 10 * 1024 * 1024 })
    log('[dsh-vibe] toolchain dependencies installed')
    return null
  } catch (error) {
...
export async function main(argv, io = {}) {
  ...
  if (command === 'upgrade' && pack.problems.length) {
    err('upgrade blocked: rule pack integrity validation failed; run `node bin/dsh-vibe.mjs hash --pack ' + packDir + '` after reviewing the sources')
    return 1
  }
  ...
  try {
    if (command === 'hash') {
      ...
      out('[dsh-vibe] hash refreshed: ' + r.changed + '/' + r.count + ' entries in ' + packDir)
    ...
    if (command === 'list-skills') {
      ...
      out('[dsh-vibe] default project skills in pack: ' + (names.length ? names.join(', ') : '(none)'))
    ...
    if (command === 'init' || command === 'upgrade') {
      ...
      out('[dsh-vibe] ' + command + ' — pack ' + pack.version + ' @ ' + pack.dir)
      ...
      if (!opts.dryRun && !r.aborted) {
        if (command === 'upgrade' && plan.stateInfo?.problem) throw new Error(plan.stateInfo.problem + '; repair or remove .dsh-vibe/state.json before retrying')
        ...
        if (!previousState || JSON.stringify(previousState) !== JSON.stringify(state)) {
          ...
          out('[dsh-vibe] state written: .dsh-vibe/state.json')
        } else {
          out('[dsh-vibe] state unchanged: .dsh-vibe/state.json')
        }
      }
    ...
    if (command === 'status' || command === 'audit') {
      ...
      if (!pPlan.stateInfo?.state) extras.push({ level: 'info', what: 'state-missing', detail: '.dsh-vibe/state.json is absent; upgrade will use legacy discovery and will not delete unknown files' })
