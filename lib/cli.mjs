// Standalone vibe-init command surface.
...
const USAGE = `vibe-init <command> [options]
...
Commands (M1, REQUIREMENTS v1.0 — project-only; the global plane is never written):
  ...
  audit            status + pack integrity + project extras (read-only)
  hash             Refresh manifest.json sha256 to current pack sources
  list-skills      List default project skills available in the pack (skills/*)
  help             Show this help

Options:
  --project <dir>   Project to act on (default: cwd, resolved upward to the .git root)
  --pack <dir>      Rule pack directory (default: <repo-root>/packages)
...
function err(msg) { console.error('[vibe-init] ' + msg) }
...
function summaryLine(summary, actionable) {
  const labels = [
    ['update', 'update'],
    ['create', 'create'],
    ['remove', 'remove'],
  ...
  const changes = labels.slice(0, 4)
    .filter(([key]) => summary[key])
    .map(([key, label]) => label + ' ' + summary[key])
  const stable = labels.slice(4)
    .filter(([key]) => summary[key])
    .map(([key, label]) => label + ' ' + summary[key])
  const parts = []
  if (changes.length) parts.push(changes.join(', '))
  else parts.push('no changes')
  if (stable.length) parts.push(stable.join(', '))
  parts.push('conflicts: ' + (summary.conflict || 0))
  return parts.join('; ') + '; actionable ' + actionable + ' item(s)'
}
...
async function reviewThenApply(label, plan, pack, opts) {
  ...
  if (opts.dryRun) {
    log('[vibe-init] ' + label + ' (dry-run): ' + summaryLine(summary, dirty.length))
  ...
  if (plan.mode === 'upgrade' && plan.stateInfo?.problem) {
    err(plan.stateInfo.problem + '; repair or remove .vibe-init/state.json before retrying')
  ...
  if (!pending.length) { log('[vibe-init] ' + label + ': nothing to do (already up to date)'); return { results, summary, applied: [], aborted: false, requiresInstall: !!plan.requiresInstall } }
  const go = opts.yes ? true : await confirm(label + ' — apply ' + pending.length + ' change(s), keep ' + blocked.length + ' conflict(s) untouched?')
  if (!go) { log('[vibe-init] ' + label + ': aborted by user — no files written'); return { results, summary, applied: [], aborted: true, requiresInstall: !!plan.requiresInstall } }
  const applied = await applyResults(results)
  log('[vibe-init] ' + label + ': applied ' + applied.length + ' change(s)')
...
export async function installToolchainIfNeeded(plan, projectRoot, runner = execFileAsync) {
  ...
  // The install home itself must resolve inside the project; its package-manager
  // content (node_modules with store symlinks) is owned by pnpm and is not walked.
  await assertRealPathContained(projectRoot, plan.toolchainHome, 'toolchain install home')
  if (process.env.VIBE_INIT_SKIP_TOOLCHAIN_INSTALL === '1') {
    log('[vibe-init] toolchain dependency install skipped by test environment')
    return null
  }
  try {
    await runner('pnpm', ['-C', plan.toolchainHome, 'install'], { cwd: projectRoot, maxBuffer: 10 * 1024 * 1024 })
    log('[vibe-init] toolchain dependencies installed')
    return null
  } catch (error) {
...
export async function main(argv, io = {}) {
  ...
  if ((command === 'init' || command === 'upgrade') && pack.problems.length) {
    err(command + ' blocked: rule pack integrity validation failed; run `vibe-init hash --pack ' + packDir + '` after reviewing the sources')
    return 1
  }
  ...
  try {
    if (command === 'hash') {
      ...
      out('[vibe-init] hash refreshed: ' + r.changed + '/' + r.count + ' entries in ' + packDir)
    ...
    if (command === 'list-skills') {
      const names = pack.skills.map((skill) => skill.name).sort()
      out('[vibe-init] default project skills in pack: ' + (names.length ? names.join(', ') : '(none)'))
    ...
    if (command === 'init' || command === 'upgrade') {
      ...
      out('[vibe-init] ' + command + ' — pack ' + pack.version + ' @ ' + pack.dir)
      ...
      if (!opts.dryRun && !r.aborted) {
        if (command === 'upgrade' && plan.stateInfo?.problem) throw new Error(plan.stateInfo.problem + '; repair or remove .vibe-init/state.json before retrying')
        try {
          await installToolchainIfNeeded(plan, projectRoot)
        } catch (error) {
          if (command === 'upgrade' && plan.previousToolchainPkg !== undefined && plan.toolchainHome) {
            // Restore the pre-install composed package.json so the next identical upgrade still
            // detects the composition change and retries the install; state is not committed.
            const pkgPath = join(plan.toolchainHome, 'package.json')
            try {
              if (plan.previousToolchainPkg === null) await unlink(pkgPath)
              else await writeFile(pkgPath, plan.previousToolchainPkg, 'utf8')
            } catch (rollbackError) {
              throw new Error('toolchain dependency install failed: ' + (error instanceof Error ? error.message : String(error))
                + '; rollback of ' + pkgPath + ' also failed: ' + (rollbackError instanceof Error ? rollbackError.message : String(rollbackError)))
            }
          }
          throw error
        }
        ...
        if (!previousState || JSON.stringify(previousState) !== JSON.stringify(state)) {
          ...
          out('[vibe-init] state written: .vibe-init/state.json')
        } else {
          out('[vibe-init] state unchanged: .vibe-init/state.json')
        }
      }
    ...
    if (command === 'status' || command === 'audit') {
      ...
      if (!pPlan.stateInfo?.state) extras.push({ level: 'info', what: 'state-missing', detail: '.vibe-init/state.json is absent; existing files without ownership records will not be deleted' })
