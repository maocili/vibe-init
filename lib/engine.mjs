...
//    conflict reported, never overwritten without --force);
...
export async function evaluatePlan(plan, opts = {}) {
  ...
  for (const e of plan.entries) {
    ...
    if (e.kind === 'file') {
      ...
      else if (opts.force || e.managedUpdate || e.managedContents?.includes(existing)) out.push(Object.assign({}, e, { action: 'update', preview: preview(e.label, existing, e.content) }))
