...
function projectTarget(projectRoot, target, label = target) {
  if (typeof target !== 'string' || target.length === 0 || target.includes('\\') || target.split('/').some((part) => !part || part === '.' || part === '..') || target.startsWith('/') || /^[a-zA-Z]:/.test(target)) {
    throw new Error('unsafe pack project target for ' + label + ': ' + JSON.stringify(target))
  }
  const targetAbs = resolve(projectRoot, target)
  assertProjectTarget(projectRoot, targetAbs, label)
  return targetAbs
}

function assertProjectTarget(projectRoot, targetAbs, label = targetAbs) {
  const rootAbs = resolve(projectRoot)
  const resolvedTarget = resolve(targetAbs)
  const rel = relative(rootAbs, resolvedTarget)
  if (rel === '..' || rel.startsWith('..' + sep) || rel.startsWith(sep)) {
    throw new Error('pack project target escapes project root for ' + label + ': ' + JSON.stringify(targetAbs))
  }
}
...
async function planInitProject(pack, projectRoot, opts = {}) {
  ...
  for (const row of pack.rows) {
    if (row.target === 'AGENTS.md') continue
    const enabled = !row.feature || (opts.features && row.feature in opts.features ? !!opts.features[row.feature] : !!pack.features[row.feature])
    const targetAbs = projectTarget(projectRoot, row.target, 'row ' + row.id)
    if (enabled) {
  ...
  if (tc) {
    const home = projectTarget(projectRoot, tc.target, 'toolchain target')
...
async function planUpgradeProject(pack, projectRoot, opts, stateInfo) {
  ...
  for (const [key, previous] of stateSegments) {
    ...
    const targetAbs = projectTarget(projectRoot, target, 'state segment ' + id)
  ...
  for (const record of state?.files || []) {
    ...
    const targetAbs = projectTarget(projectRoot, record.path, 'state file ' + record.path)
    let present = false
    try { present = (await stat(targetAbs)).isFile() } catch { }
...
export async function evaluatePlan(plan, opts = {}) {
  ...
  for (const e of plan.entries) {
    assertProjectTarget(plan.projectRoot, e.targetAbs, e.label)
