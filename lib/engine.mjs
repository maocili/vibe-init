...
async function annotateInitOwnership(pack, projectRoot, plan) {
  ...
  for (const entry of plan.entries) {
    ...
    else if (entry.id.startsWith('skill:')) entry.sourceRel = 'skills-optional/' + entry.id.slice('skill:'.length).replace(/^[^:]+:/, (m) => m.slice(0, -1) + '/')
  ...
  for (const name of wanted) {
    ...
    try {
      for (const f of await listFiles(src)) {
        ...
        const targetAbs = join(projectRoot, '.agents', 'skills', name, f.rel)
        addFile(annotateFile({ kind: 'file', id: 'skill:' + name + ':' + f.rel, targetAbs, content, label: statePath(projectRoot, targetAbs), upgradeManaged: true }, projectRoot, 'skill', 'skills-optional/' + name + '/' + f.rel, contentSha256(raw)))
      }
