// dsh-vibe materialization engine (M1, REQUIREMENTS v1.0 — project-only initializer).
...
//  • the docGates toolchain under <project>/.dsh-vibe/toolchain (umbrella feature +
...
export function defaultPackDir() {
  const env = process.env.DSH_VIBE_PACK
...
function startMarker(id) { return '<!-- dsh-vibe:' + id + ':start -->' }
function endMarker(id) { return '<!-- dsh-vibe:' + id + ':end -->' }
...
export function composeToolchainPackageJson(tc, enabledGroupIds) {
  ...
  scripts['doc-sync'] = verifyNames.length
    ...
    : 'echo "no doc gates enabled (dsh-vibe: enable a gate group)"'
  const body = {
    name: pg.name || 'dsh-vibe-toolchain',
...
function staleFileEntry(projectRoot, record, targetAbs, note) {
  return {
    ...
    label: record.path,
    home: join(projectRoot, '.dsh-vibe'),
...
async function annotateInitOwnership(pack, projectRoot, plan) {
  ...
  for (const entry of plan.entries) {
    ...
    else if (rel.startsWith('.dsh-vibe/toolchain/')) entry.ownership = 'toolchain'
...
async function planUpgradeProject(pack, projectRoot, opts, stateInfo) {
  ...
  // previous state proves that this marker was installed by dsh-vibe; unknown markers stay
  ...
  if (tc) {
    ...
    // guessing whether they are user additions or obsolete dsh-vibe output.
    if (!state) {
      try {
...
export async function auditExtras(pack, projectRoot) {
  ...
  for (const row of pack.rows) {
    if (row.declaredSha256 && row.declaredSha256 !== row.actualSha256) {
      notes.push({ level: 'warn', what: 'pack-drift', detail: row.id + ': manifest sha256 stale (re-run "dsh-vibe hash")' })
    }
  }
  if (projectRoot) {
