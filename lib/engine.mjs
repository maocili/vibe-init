// vibe-init materialization engine (M1, REQUIREMENTS v1.0 — project-only initializer).
...
//  • the docGates toolchain under <project>/.vibe-init/toolchain (umbrella feature +
...
import { cleanSource, isSafeRelativePath, listFiles } from './pack.mjs'
...
/** Pack location convention: <repo-root>/packages next to the runtime source. */
export function defaultPackDir() {
  const env = process.env.VIBE_INIT_PACK
...
function startMarker(id) { return '<!-- vibe-init:' + id + ':start -->' }
function endMarker(id) { return '<!-- vibe-init:' + id + ':end -->' }

function inspectMarkerText(existing) {
  const text = existing == null ? '' : String(existing)
  const comments = text.matchAll(/<!--\s*vibe-init:[\s\S]*?(?:-->|$)/g)
  const exact = /^<!-- vibe-init:([a-zA-Z0-9._-]+):(start|end) -->$/
  const segments = new Map()
  let open = null
  let malformed = false
  for (const comment of comments) {
    const parsed = exact.exec(comment[0])
    if (!parsed) { malformed = true; continue }
    const [, id, boundary] = parsed
    if (boundary === 'start') {
      if (open) malformed = true
      open = { id, start: comment.index }
      continue
    }
    if (!open || open.id !== id) {
      malformed = true
    ...
    if (segments.has(id)) malformed = true
    segments.set(id, text.slice(open.start, comment.index + comment[0].length))
    open = null
  }
  if (open) malformed = true
  return { malformed, segments }
}
...
async function projectTarget(projectRoot, target, label = target) {
  if (!isSafeRelativePath(target)) {
    throw new Error('unsafe pack project target for ' + label + ': ' + JSON.stringify(target))
  }
  return await assertProjectTarget(projectRoot, resolve(projectRoot, target), label)
}

async function assertProjectTarget(projectRoot, targetAbs, label = targetAbs) {
  try {
    return await assertRealPathContained(projectRoot, targetAbs, label)
  } catch (error) {
    throw new Error('project target confinement failed for ' + label + ': ' + (error instanceof Error ? error.message : String(error)))
  }
}
...
export function composeToolchainPackageJson(tc, enabledGroupIds) {
  ...
  scripts['doc-sync'] = verifyNames.length
    ...
    : 'echo "no doc gates enabled (vibe-init: enable a gate group)"'
  const body = {
    name: pg.name || 'vibe-init-toolchain',
...
/** Whether existing text is this tool's own composed toolchain package (same declared name). */
function isComposedToolchainPackageJson(tc, text) {
  try {
    const parsed = JSON.parse(String(text))
    const expected = (tc?.packageJson?.name || 'vibe-init-toolchain')
    return parsed && typeof parsed === 'object' && parsed.name === expected
  } catch {
    return false
  }
}
...
async function planInitProject(pack, projectRoot, opts = {}) {
  ...
  let skeletonFiles = []
  try {
    skeletonFiles = await listFiles(skeletonDir, '', pack.dir)
  } catch (error) {
  ...
  for (const f of skeletonFiles) {
    ...
    for (const d of relDirs) {
      acc = join(acc, d)
      await assertProjectTarget(projectRoot, acc, relative(projectRoot, acc))
    ...
    const alternateManagedContents = f.rel === 'README.md'
      ...
      : null
    const targetAbs = await projectTarget(projectRoot, '.agents/notes/' + f.rel, 'notes skeleton ' + f.rel)
  ...
  entries.unshift({ kind: 'dir', targetAbs: await projectTarget(projectRoot, '.agents/notes', 'notes root'), label: '.agents/notes' })
  ...
  for (const row of pack.rows) {
    ...
    entries.push({
      kind: 'segment', id: row.id, targetAbs: await projectTarget(projectRoot, 'AGENTS.md', 'row ' + row.id),
  ...
  for (const row of pack.rows) {
    if (row.target === 'AGENTS.md') continue
    const enabled = !row.feature || (opts.features && row.feature in opts.features ? !!opts.features[row.feature] : !!pack.features[row.feature])
    const targetAbs = await projectTarget(projectRoot, row.target, 'row ' + row.id)
    if (enabled) {
  ...
  for (const name of wanted) {
    ...
    try {
      const files = await listFiles(src, '', pack.dir)
      for (const f of files) {
        const content = cleanSource(await readFile(f.abs, 'utf8'))
        const targetAbs = await projectTarget(projectRoot, '.agents/skills/' + name + '/' + f.rel, 'skill ' + name + '/' + f.rel)
  ...
  if (tc) {
    const home = await projectTarget(projectRoot, tc.target, 'toolchain target')
    ...
    for (const g of tc.groups) {
      ...
      for (const f of g.files) {
        ...
        const targetAbs = await projectTarget(projectRoot, label, 'toolchain ' + g.id + '/' + f.rel)
        if (on) {
    ...
    const pkgPath = await projectTarget(projectRoot, tc.target + '/package.json', 'toolchain package.json')
    if (umbrella) {
      const pkg = composeToolchainPackageJson(tc, enabledGroups)
      const existingPkg = await readText(pkgPath)
      // init may refresh its own derived package.json (composition changes), but a foreign
      // package.json in the managed home is a conflict, never an overwrite. A file counts as
      // derived when it is the toolchain composition (same declared package name).
      const derived = existingPkg !== null && isComposedToolchainPackageJson(tc, existingPkg)
      entries.push({ kind: 'file', id: 'toolchain:package.json', targetAbs: pkgPath, content: pkg, label: join(tc.target, 'package.json'), group: 'package.json', managedUpdate: derived })
    } else {
...
function staleFileEntry(projectRoot, record, targetAbs, note) {
  return {
    ...
    label: record.path,
    home: join(projectRoot, '.vibe-init'),
...
async function annotateInitOwnership(pack, projectRoot, plan) {
  ...
  for (const entry of plan.entries) {
    ...
    else if (rel.startsWith('.vibe-init/toolchain/')) entry.ownership = 'toolchain'
...
async function planUpgradeProject(pack, projectRoot, opts, stateInfo) {
  ...
  let skeletonFiles = []
  try {
    skeletonFiles = await listFiles(join(pack.dir, 'notes-skeleton'), '', pack.dir)
  } catch (error) {
    if (!error || error.code !== 'ENOENT') throw error
  }
  for (const f of skeletonFiles) {
  ...
  for (const row of pack.rows) {
    ...
    entries.push({ kind: 'segment', id: row.id, targetAbs: await projectTarget(projectRoot, 'AGENTS.md', 'row ' + row.id), target: 'AGENTS.md', body, label: 'AGENTS.md (segment ' + row.id + ')', feature: row.feature, enabled, ownership: 'rule-segment', sourceRel: row.rel, sourceSha256: row.actualSha256, installedSha256: contentSha256(makeSegment(row.id, body)), upgradeManaged: true })
  }

  // A rule segment can disappear or be renamed in a newer pack. Remove it only when the
  // previous state proves that this marker was installed by vibe-init; unknown markers stay
  ...
  for (const [key, previous] of stateSegments) {
    ...
    const target = previous.target || 'AGENTS.md'
    const targetAbs = await projectTarget(projectRoot, target, 'state segment ' + id)
    const existing = await readText(targetAbs)
    const inspected = inspectMarkerText(existing)
    const installed = inspected.segments.get(id)
    if (inspected.malformed || !installed || contentSha256(installed) !== previous.installedSha256) {
      entries.push({
        kind: 'ownership-conflict', id: 'stale-segment:' + id, targetAbs,
        label: target + ' (segment ' + id + ')', ownership: 'rule-segment',
        note: 'state-owned marker is missing, malformed, or differs from its installed form; kept'
      })
      continue
    }
    staleSegments.push({ ...previous, targetAbs, id })
    entries.push({
      kind: 'segment', id, targetAbs, target, body: null,
      label: target + ' (segment ' + id + ')',
      note: 'no longer declared by current rule pack', ownership: 'rule-segment',
      expectHash: previous.installedSha256
    })
  }
  ...
  for (const name of wanted) {
    const src = join(skillsDir, name)
    try {
      for (const f of await listFiles(src, '', pack.dir)) {
  ...
  if (tc) {
    ...
    for (const g of tc.groups) {
      ...
      if (on) enabledGroups.add(g.id)
      for (const f of g.files) {
        const targetAbs = await projectTarget(projectRoot, tc.target + '/' + f.rel, 'toolchain ' + g.id + '/' + f.rel)
        if (on) {
          const content = await readText(f.abs)
    ...
    if (umbrella) {
      ...
      // Pre-install bytes of the composed package.json. When dependency installation fails, the
      // CLI restores this file so a retried upgrade still detects the composition change and
      // retries the install instead of committing state with dependencies missing.
      previousPkgContent = existing
    } else if (!state) {
      const existing = await readText(pkgPath)
    ...
    // guessing whether they are user additions or obsolete vibe-init output.
    if (!state) {
      try {
  ...
  for (const record of state?.files || []) {
    ...
    const targetAbs = await projectTarget(projectRoot, record.path, 'state file ' + record.path)
    let present = false
    try { present = (await stat(targetAbs)).isFile() } catch { }
  ...
  return {
    ...
    previousToolchainPkg: previousPkgContent,
...
export async function planProject(pack, projectRoot, opts = {}) {
  ...
  await projectTarget(projectRoot, '.vibe-init/state.json', 'project state')
...
export async function evaluatePlan(plan, opts = {}) {
  ...
  for (const e of plan.entries) {
    if (e.targetAbs) await assertProjectTarget(plan.projectRoot, e.targetAbs, e.label)
    const safeEntry = Object.assign({}, e, { projectRoot: plan.projectRoot })
    if (e.kind === 'dir') { out.push(Object.assign(safeEntry, { action: 'dir' })); continue }
    if (e.kind === 'missing-skill') { out.push(Object.assign(safeEntry, { action: 'missing' })); continue }
    if (e.kind === 'ownership-conflict') { out.push(Object.assign(safeEntry, { action: 'conflict' })); continue }
    const key = e.targetAbs + '|' + e.id
    if (seen.has(key)) { out.push(Object.assign(safeEntry, { action: 'skip', note: 'duplicate target' })); continue }
    seen.add(key)
    ...
    const newHash = e.kind === 'file'
      ...
      : e.kind === 'segment' && e.body !== null
        ...
        : null
    const result = (extra) => Object.assign({}, safeEntry, { oldHash, newHash }, extra)
    if (e.kind === 'file') {
    ...
    if (inspected.malformed) {
      out.push(result({ action: 'conflict', note: 'marker is missing, duplicated, malformed, or cross-structured' }))
      continue
    }
    if (e.body === null && e.expectHash) {
      const installed = inspected.segments.get(e.id)
      if (!installed || contentSha256(installed) !== e.expectHash) {
        out.push(result({ action: 'conflict', note: 'state-owned marker is missing or differs from its installed form; kept' }))
        continue
      }
    }
...
export async function applyResults(results) {
  ...
  for (const r of results) {
    if (r.targetAbs) await assertProjectTarget(r.projectRoot, r.targetAbs, r.label)
    if (r.kind === 'dir') {
  ...
  for (const [targetAbs, segs] of segmentGroups) {
    await assertProjectTarget(segs[0].projectRoot, targetAbs, segs[0].label)
    let text = await readTarget(targetAbs)
    if (text == null) text = ''
    const changed = []
    for (const s of segs) {
      if (s.body === null && s.expectHash) {
        const inspected = inspectMarkerText(text)
        const installed = inspected.segments.get(s.id)
        if (inspected.malformed || !installed || contentSha256(installed) !== s.expectHash) {
          s.action = 'conflict'
          s.note = 'state-owned marker changed after evaluation; kept'
          continue
        }
      }
...
export async function auditExtras(pack, projectRoot) {
  ...
  for (const row of pack.rows) {
    if (row.declaredSha256 && row.declaredSha256 !== row.actualSha256) {
      notes.push({ level: 'warn', what: 'pack-drift', detail: row.id + ': manifest sha256 stale (re-run "vibe-init hash")' })
    }
  }
  if (projectRoot) {
