// dsh-vibe pack loader — reads a versioned rule pack (manifest.json + source files).
...
// are materialized (planning headers "[实现期]" and legacy "dsh-vibe:*" marker lines).
...
// and legacy "dsh-vibe:*" marker lines) never reach consumers: they are stripped wherever they
// sit (top, mid-file, bottom). The engine re-wraps every managed segment in its own id markers.
const SINGLE_MARKER_RE = /^<!--\s*dsh-vibe(?::[a-zA-Z0-9._-]+)?\s*(?::start|:end)?\s*-->$/
const PLANNING_RE = /待填充|\[实现期\]|^<!--\s*占位|^<!--\s*dsh-vibe:feature/
