// Project-local ownership state for dsh-vibe upgrade migrations.
// This file describes plugin-owned paths only; user notes and user-created files are never listed.
import { createHash } from 'node:crypto'
import { mkdir, readFile, rename, writeFile } from 'node:fs/promises'
import { join } from 'node:path'

export const STATE_SCHEMA_VERSION = 1
export const STATE_RELATIVE_PATH = '.dsh-vibe/state.json'
...
function safeManagedFileRecord(record) {
  ...
  const allowed = record.kind === 'generated-doc' && path === 'docs/AGENTS.md'
    || record.kind === 'toolchain' && path.startsWith('.dsh-vibe/toolchain/')
...
export async function readProjectState(projectRoot) {
  ...
  try {
    ...
    if (!Array.isArray(parsed.files) || !Array.isArray(parsed.segments)) {
      return { path, state: parsed, problem: 'state.json must contain files and segments arrays' }
    }
    if (parsed.files.some((record) => !safeManagedFileRecord(record))
      || parsed.segments.some((record) => !safeSegmentRecord(record))) {
      return { path, state: parsed, problem: 'state.json contains an unsafe or unowned managed record' }
    }
...
export async function writeProjectState(projectRoot, state) {
  ...
  await mkdir(join(projectRoot, '.dsh-vibe'), { recursive: true })
