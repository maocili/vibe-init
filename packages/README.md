...
> （含有效 pairing record）、英文单语指针式根块、textLinkManagement / bilingualDocsDiscipline / docBudgets 特性段、
...
| `standing-orders-block.md` | `<project>/AGENTS.md`（marker 包裹追加） | 英文单语项目常设契约（指针式，指向项目笔记骨架门规） |
...
| 规则族 | 落点 | 默认 |
|---|---|---|
| agent note（何时需要记录） | 根 `AGENTS.md` 单句契约 + `notes-skeleton/AGENTS.md`、`README*.md` 中的详细规则 | 开 |
...
| 双语纪律段（md/zh 配对同步纪律，重机制） | 根 `AGENTS.md` 的英文单句契约（`features/bilingual-docs.md`）；门禁+工具链+语料 = toolchain bilingual 组 | 关（独立 key `bilingualDocsDiscipline`） |
