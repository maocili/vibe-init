...
> This standalone example describes a modular tool architecture. The governing principle, from the microkernel design discussion: **everything is a plugin**. The core is deliberately tiny — a handful of abstract services plus one concrete loop — and every feature is a plugin against the extension API described here, without modifying the loop.

本文介绍一个模块化工具架构示例。核心设计准则是：**一切皆插件**。内核刻意做得极精简，仅包含少量抽象服务，外加一个实体循环；所有功能均基于本文定义的扩展接口开发为独立插件，无需改动主循环逻辑。

> Dependency rule: extension plugins depend on interfaces, never on the concrete loop (the loop is swappable); a composition bundle may assemble the concrete spine when needed.

依赖约束规范：各类扩展插件仅依赖抽象接口，不直接依赖实体主循环（主循环支持替换实现）；必要时可由组合包负责组装实体主干。

> This document covers **behavior**; type definitions live in subsystems/, the per-event/service reference lives in the generated regions of subsystems/, and package contracts in the package READMEs state each package's required configuration and behavior (map).

本文档描述整体行为逻辑；类型定义存放于 subsystems/；各类事件、服务的详细参考见 subsystems/ 中的生成区块；相应的 README 说明每个包（package）要求的配置和行为（索引）。
...
- 代码体标识符（事件名 `agent/status`、状态值 `running`、包名 `local-shell` 等）在译文中保留 code span 原文，不得口语化改写；Pass 2 必须逐句核验。
