#!/usr/bin/env python3
"""Encode lexically ordered screenshots into a verified GIF."""
...
def run_json(command: list[str]) -> dict[str, object]:
    try:
        ...
        fail(error.stderr.strip() or error.stdout.strip() or str(error))
    try:
...
def probe_stream(ffprobe: str, path: Path) -> dict[str, object]:
    result = run_json([
        ...
        "-show_entries", "stream=width,height,nb_frames,duration", "-of", "json", str(path),
    ])
...
def stream_int(stream: dict[str, object], key: str, path: Path) -> int:
    ...
    if value <= 0:
        fail(f"non-positive integer {key!r} in media probe for {path}")
    return value
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--durations", default="2", help="one hold duration or one value per source frame")
    parser.add_argument("--fps", type=positive_int, default=10, help="encoded frames per second")
    parser.add_argument("--max-width", type=positive_int, default=1200, help="maximum output width")
    parser.add_argument("--colors", type=positive_int, default=128, help="palette colors, from 4 through 256")
    parser.add_argument("--max-bytes", type=positive_int, default=DEFAULT_MAX_BYTES, help="maximum output size")
...
def main() -> None:
    ...
    dimensions = {
        ...
        for frame in frames for stream in [probe_stream(ffprobe, frame)]
    }
    if len(dimensions) != 1:
    ...
    with tempfile.TemporaryDirectory(prefix="record-browser-gif-") as temporary:
        ...
        try:
            subprocess.run([
                ...
                "-i", str(manifest), "-vf", filters, "-loop", "0", "-t", f"{expected_duration:.6f}",
                "-y" if args.force else "-n", str(output),
            ], check=True)
        except subprocess.CalledProcessError as error:
            fail(f"ffmpeg failed with exit code {error.returncode}")

    stream = probe_stream(ffprobe, output)
    width, height = stream_int(stream, "width", output), stream_int(stream, "height", output)
    ...
    if abs(actual_duration - expected_duration) > max(0.2, 2 / args.fps):
    ...
    if byte_size > args.max_bytes:
        fail(f"output is {byte_size} bytes, above --max-bytes")
    print(json.dumps({
        "output": str(output), "sourceFrames": len(frames), "encodedFrames": encoded_frames,
        "width": width, "height": height, "durationSeconds": actual_duration,
        "fps": args.fps, "bytes": byte_size,
